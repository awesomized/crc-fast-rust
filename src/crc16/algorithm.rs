...
//! using the existing CRC-32 algorithm infrastructure, and then scaling the
//! result back to 16 bits.
...
impl EnhancedCrcWidth for crate::structs::Width16 {
    ...
    unsafe fn fold_16<T: ArchOps>(
        ...
        T::Vector: Copy,
    {
        // CRC-16 uses the same folding as CRC-32
    ...
    where
        T::Vector: Copy,
    {
        // CRC-16 uses the same fold_width as CRC-32
    ...
    where
        T::Vector: Copy,
    {
        // CRC-16 uses the same Barrett reduction as CRC-32, then extracts 16 bits
        let x = state.value;
        ...
        if state.reflected {
            ...
            ops.extract_u64s(xorred)[1] as u16
        } else {
            ...
            // Scale back from 32 bits to 16 bits
            ((ops.extract_u64s(final_xor)[0] >> 32) >> 16) as u16
        }
    }
    ...
    where
        T::Vector: Copy,
    {
        // CRC-16 uses non-reflected coefficient creation like CRC-32
    ...
    fn get_last_bytes_table_ptr(reflected: bool, remaining_len: usize) -> (*const u8, usize) {
        // CRC-16 uses the same shuffle tables as CRC-32
        if reflected {
